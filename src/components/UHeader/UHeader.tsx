...
})<{ bgColor?: string }>(({ theme, color, bgColor }) => ({
  [`& .${classes.bgColor}`]: {
...
export interface UHeaderProps
  extends Omit<AppBarProps, 'position' | 'elevation' | 'children'> {
  ...
  toggleDrawer?: (event: React.MouseEvent, open: boolean) => void
...
export default function UHeader(props: UHeaderProps) {
  ...
  return (
    <StyledAppBar position={position} elevation={elevation} bgColor={bgColor}>
      <Toolbar disableGutters={true} className={classes.bgColor}>
        ...
        >
          {showHamburgerMenu !== false && (
            <Box sx={{ mr: 2 }}>
              ...
              >
                {findReactChildren({ children }, UHeaderLeftMenu)}
              </Drawer>
            </Box>
          )}
          {(!hideLogo && (
          ...
          <Box className={classes.navbarCenter}>
            {findReactChildren({ children }, UNavbarCenter)}
          </Box>
        </Box>
        <Box
          sx={{
        ...
        >
          {findReactChildren({ children }, UHeaderRightButtons)}
        </Box>
      </Toolbar>
      {findReactChildren({ children }, UHeaderMainMenu)}
    </StyledAppBar>
  )
}
