...
function ForwardRefForm(props, ref) {
  ...
  ValidatorForm.addValidationRule('isSafeText', value => {
    ...
    if (value != '' && htmlRegexp.test(value)) {
      return false
    }
    return true
  })
  ...
  ValidatorForm.addValidationRule('isAlphanumeric', value => {
    // With Unicode Property Escapes, you can match a letter from any language with the following simple regular expression (https://javascript.info/regexp-unicode) => /\p{Letter}/u
    // Unicode Property Escapes can be transpiled down to ES5 with a tool called regexpu.
    // The following is transpiled code for all language char with space and numbers ( taken from https://stackoverflow.com/a/54879743/1874080 with some modifications) :
    ...
    return value != '' && wordsInAllLanguages.test(value)
  })
